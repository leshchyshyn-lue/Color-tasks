...
public interface TaskRepository extends JpaRepository<Task, Integer> {
    ...
    @Transactional
    @Modifying
    @Query(value = "DELETE FROM task WHERE user_id = ?;", nativeQuery = true)
    void deleteAllTasksByUserId(int id);

    @Transactional
    @Modifying
    @Query(value = "DELETE FROM task WHERE color = ? AND user_id = ?", nativeQuery = true)
    void deleteTasksByColor(String color, int id);


    @Query(value = "SELECT * FROM task WHERE user_id = ? AND color = ?", nativeQuery = true)
